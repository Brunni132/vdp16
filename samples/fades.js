...
const TextLayer = {
	...
	drawLayer: function(opacity) {
		...
		this.vdp.configBackgroundTransparency({ op: 'add', blendDst: '#fff', blendSrc: color32.make(opacity, opacity, opacity) });
...
function *main(vdp) {
	...
	const fadeCustomStatus = new Array(palette1Original.buffer.length);
	const FADE_LIST = [
